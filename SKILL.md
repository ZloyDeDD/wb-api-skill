...
Снимок старше 30 дней — скажи об этом пользователю прямо: назови дату снимка и предложи `python scripts/update.py`. Не делай вид, что данные свежие.
...
`python scripts/update.py` — качает свежие спеки напрямую с dev.wildberries.ru, проверяет результат до записи и дописывает в `CHANGELOG.md`, что изменилось в API. Подробности в README.md.
